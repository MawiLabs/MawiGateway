...
use mawi_core::agentic::{AgenticConfig, AgenticMessage, Tool, ToolCall, ToolResult, ToolType};
use mawi_core::unified::{ChatMessage, UnifiedChatRequest, UnifiedChatResponse};
use sqlx::PgPool;
...
impl AgenticExecutor {
    ...
    {
        Box::pin(async move { self.execute_internal(request, user_id).await })
    }
    ...
    async fn execute_internal(
        ...
        let config = self
            .load_agentic_config(&request.service)
            .await
            .map_err(|e| {
        ...
        info!(
            "📋 CONFIG LOADED SUCCESS: planner={}, tools={}",
            config.planner_model_id,
            config.tools.len()
        );
        ...
        let mut plan = self
            .generate_plan(&config, request, &memory, false, user_id)
            .await
            .map_err(|e| {
        ...
        if plan.is_empty() {
            let user_content = request
                ...
                .unwrap_or_default();
            if user_content.contains("image")
                || user_content.contains("generate")
                || user_content.contains("draw")
                || user_content.contains("create")
            {
                ...
                if let Some(tool) = config
                    .tools
                    .iter()
                    .find(|t| t.tool_type == ToolType::ImageGeneration)
                {
                    plan = vec![
                        format!(
                            "TOOL[{}](Imagine and generate: {})",
                            tool.name,
                            request
                                .messages
                                .last()
        ...
        if plan.is_empty() {
            ...
            return self
                .execute_react_loop(&config, request, &dummy_memory, user_id)
                .await;
        }
        ...
        if plan.is_empty() {
            ...
            return self
                .execute_react_loop(&config, request, &memory, user_id)
                .await;
        }
        ...
        for (i, step) in plan.iter().enumerate() {
            ...
            let result = self
                .execute_react_loop(&config, &step_request, &memory, user_id)
                .await?;
            ...
            let content = result
                ...
                .unwrap_or_else(|| "No content result".to_string());
            memory.add(format!("Step {}: {}", i + 1, step), content);
        }

        // 4. Final Synthesis (Make an answer with the output of the script)
        info!(
            "✨ Synthesizing final answer from {} context items",
            memory.entries.len()
        );
        let final_response = self
            .synthesize_answer(&config, request, &plan, &memory, user_id)
            .await?;
    ...
    async fn synthesize_answer(
        ...
        let mut response = self
            .executor
            .execute_model_directly(&config.planner_model_id, messages, user_id, None)
            .await?;
        let context_str = memory.get_context();
        ...
        if let Some(choice) = response.choices.first_mut() {
            for (full_markdown, url) in images_found {
                        ...
                        if idx >= 2 {
                            ...
                            if prefix == "](" {
                                ...
                                if let Some(last_open) = content[..idx - 2].rfind('[') {
                                    ...
                                    if last_open > 0 && &content[last_open - 1..last_open] == "!" {
                ...
                debug!(
                    "🖼️ Image Check for {}: Present={}, IsImage={}",
                ...
                if !is_image_format {
                    info!(
                        "🖼️ Model missed image or format, appending: {}",
                        full_markdown
                    );
    ...
    async fn generate_plan(
        ...
        let messages = vec![
            ...
            ChatMessage {
                role: "user".to_string(),
                content: format!("USER REQUEST TO PLAN: '{}'", user_query),
            },
        ];
    ...
    fn parse_plan_content(content: &str) -> Vec<String> {
        ...
        clean_content
            ...
            .filter(|l| {
                ...
                !(low.contains("can't create")
                    || low.contains("cannot generate")
                    || low.contains("don't have the ability"))
            })
            .map(|l| {
                let trimmed = l.trim();
    ...
    fn build_step_request(
        &self,
        original_request: &UnifiedChatRequest,
        step: &str,
        memory: &ShortTermMemory,
    ) -> UnifiedChatRequest {
    ...
    async fn execute_react_loop(
        ...
        let mut generated_images: std::collections::HashSet<String> =
        ...
        for iteration in 0..config.max_iterations {
            ...
            let _planner_content = planner_response
                .choices
                .first()
                .map(|c| c.message.content.clone())
                .unwrap_or_default();
            ...
            if let Some(tool_calls) = self.extract_tool_calls(&planner_response) {
                ...
                messages.push(AgenticMessage::assistant_with_tool_calls(
                    tool_calls.clone(),
                ));
                ...
                for call in &tool_calls {
                    ...
                    if is_image_tool && !generated_images.is_empty() {
                        ...
                        if (call.name.contains("generate_image")
                            || call.name.contains("dall-e")
                            || call.name.contains("stable_diffusion"))
                            && generated_images.contains(&call.arguments)
                        {
                            debug!(
                                "⚠️ Skipping duplicate image generation - already have {} image(s)",
                                generated_images.len()
                            );
                            continue;
                        }
                        if let Some(existing_url) = generated_images.iter().next() {
                            messages.push(AgenticMessage::tool_result(
                                &call.id,
                                format!(
                                    "Image already generated: ![Generated Image]({})",
                                    existing_url
                                ),
                            ));
                            continue;
        ...
        Err(anyhow!(
            "Max iterations ({}) exceeded without final answer",
            config.max_iterations
        ))
    }
    ...
    async fn load_agentic_config(&self, service_name: &str) -> Result<AgenticConfig> {
        ...
        let mut tools: Vec<Tool> = tools_rows
            .into_iter()
            .map(
                |(id, name, description, tool_type_str, target_id, params_schema): (
                    String,
                    String,
                    String,
                    String,
                    ...
                    let tool_type = ToolType::try_from(tool_type_str).unwrap_or(ToolType::Model);

                    let parameters_schema = params_schema
                        .as_deref()
        ...
        for (id, server_id, name, description, input_schema, server_name) in mcp_tools_rows {
            ...
            let server_name_sanitized = server_name
                .to_lowercase()
                .replace(" ", "_")
                .replace("-", "_");
        ...
        let assigned_models: Vec<(String, String, String, String)> =
            sqlx::query_as::<sqlx::Postgres, (String, String, String, String)>(
        ...
        for (id, name, modality, target_id) in assigned_models {
            ...
            debug!(
                "🛠️ Auto-mapping model '{}' (modality: {}) to tool: {}",
                id, modality, final_tool_name
            );
            tools.push(Tool {
        ...
        if has_specific_image {
            eprintln!(
                "🧹 Removing generic 'image_generation' tool because 'generate_image' exists"
            );
        ...
        if has_specific_video {
            eprintln!(
                "🧹 Removing generic 'video_generation' tool because 'generate_video' exists"
            );
    ...
    fn build_initial_messages(
        ...
        let image_tool = config
            .tools
            .iter()
            .find(|t| t.tool_type == ToolType::ImageGeneration);
        let video_tool = config
            .tools
            .iter()
            .find(|t| t.tool_type == ToolType::VideoGeneration);
        let audio_tool = config
            .tools
            .iter()
            .find(|t| t.tool_type == ToolType::TextToSpeech);
    ...
    async fn call_planner_stream(
        ...
        std::pin::Pin<
            Box<dyn futures::Stream<Item = Result<mawi_core::unified::AgenticStreamEvent>> + Send>,
        >,
    > {
        ...
        let json_mode = Some(mawi_core::types::ResponseFormat {
            type_: "json_object".to_string(),
        });
        self.executor
            .execute_model_stream_directly(
                &config.planner_model_id,
                unified_messages,
                user_id,
                json_mode,
            ...
            if let Some(tools_array) = json_val.get("tools").and_then(|v| v.as_array()) {
                ...
                for tool_obj in tools_array {
                    if let (Some(name), Some(args)) = (
                        tool_obj.get("name").and_then(|s| s.as_str()),
    ...
    fn parse_simple_tool_calls(&self, content: &str) -> Option<Vec<ToolCall>> {
        ...
        for cap in re.captures_iter(content) {
            if let (Some(name_match), Some(args_match)) = (cap.get(1), cap.get(2)) {
                ...
                let arguments = if args_trimmed.starts_with('{')
    ...
    async fn execute_tool(
        ...
        let matches: Vec<&Tool> = config
            ...
            .filter(|t| {
                ...
                // 2. Case insensitive
                if t.name.eq_ignore_ascii_case(&call.name) {
                    return true;
                }
                ...
                if call.name.chars().all(char::is_alphanumeric)
                    && t.name.to_lowercase().contains(&call.name.to_lowercase())
                {
                    return true;
                }
                false
        ...
        let tool = match matches.len() {
            0 => {
                ...
                if lower_name.contains("image")
                    || lower_name.contains("gen")
                    || lower_name.contains("draw")
                    || lower_name.contains("picture")
                    || lower_name.contains("solimg")
                    || lower_name.contains("dall")
                {
                    ...
                    if let Some(img_tool) = config.tools.iter().find(|t| {
                        ...
                        info!(
                            "✨ Smart Resolve: Mapped generic '{}' to tool '{}'",
                            call.name, img_tool.name
                        );
                        img_tool
                    } else {
            ...
            _ => {
                ...
                let candidates = matches
                    ...
                    .join(", ");
                warn!(
                    "⚠️ Ambiguous tool call '{}' matched multiple tools: {}",
                    call.name, candidates
                );
                return Ok(ToolResult {
                    tool_call_id: call.id.clone(),
                    content: format!(
                        "Ambiguous tool name '{}'. Did you mean one of: {}? Please be specific.",
                        call.name, candidates
                    ),
                    success: false,
        ...
        let result: Result<String> = match tool.tool_type {
            ...
            ToolType::Service => {
                self.execute_service_tool(&tool.target_id, &args, user_id)
                    .await
            }
            ToolType::ImageGeneration => {
                self.execute_image_generation_tool(&tool.target_id, &args, user_id)
            ...
            ToolType::TextToSpeech => self.execute_tts_tool(&tool.target_id, &args, user_id).await,
            ToolType::SpeechToText => self.execute_stt_tool(&tool.target_id, &args, user_id).await,
            ToolType::Mcp => {
                ...
                info!(
                    "🔌 Executing MCP Tool via Manager: server={}, tool={}",
                    server_id, tool_name
                );
                ...
                if should_apply_heuristic {
                    if let Some(obj) = final_args.as_object_mut() {
                        ...
                        obj.remove("input");
                        info!(
                            "🔧 Applied Argument Heuristic: 'input' -> owner/repo ({}/{})",
                            owner, repo
                        );
                    }
                }
    ...
    async fn execute_model_tool(
        ...
        let input = args
            ...
            .ok_or_else(|| {
                anyhow!(
                    "Missing 'input' (or 'query'/'arg'/'prompt') in tool arguments. Got: {:?}",
                    args
                )
            })?;
        ...
        let response = self
            .executor
            .execute_model_directly(model_id, messages, user_id, None)
            .await?;
        Ok(response
            .choices
            .first()
    ...
    async fn execute_image_generation_tool(
        &self,
        model_id: &str,
        args: &serde_json::Value,
        user_id: &str,
        ...
        let response = self
            .executor
            .execute_image_generation(&request, user_id)
            .await?;
    ...
    async fn execute_video_generation_tool(
        &self,
        model_id: &str,
        args: &serde_json::Value,
        user_id: &str,
    ...
    async fn execute_tts_tool(
        ...
        let (content_type, audio_bytes): (String, Vec<u8>) = self
            .executor
            .execute_text_to_speech(&request, user_id)
            .await?;
        Ok(format!(
    ...
    async fn execute_stt_tool(
        ...
        Err(anyhow!(
            "Speech-to-text not supported in current tool calling implementation"
        ))
    }
}
