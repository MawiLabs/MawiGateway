...
use mawi_core::auth::{AuthService, LoginRequest, RegisterRequest};
use poem::http::header;
use poem::{http::StatusCode, Request, Response, Result as PoemResult};
use poem_openapi::{payload::Json, payload::Response as PoemApiResponse, Object, OpenApi};
...
impl AuthApi {
    ...
    pub async fn register(
        ...
        match auth_service.register(register_req).await {
            Ok(auth_response) => {
                ...
                let user_json_str =
                    serde_json::to_string(&json!({ "user": user_json })).map_err(|e| {
    ...
    pub async fn login(&self, req: Json<LoginReq>) -> PoemResult<PoemApiResponse<Json<Value>>> {
        let auth_service = AuthService::new(self.pool.clone());
    ...
    pub async fn logout(&self, req: &Request) -> PoemResult<PoemApiResponse<Json<String>>> {
        if let Some(token) = get_session_token(req) {
            let auth_service = AuthService::new(self.pool.clone());
            let _ = auth_service.logout(&token).await;
        }
        Ok(
            PoemApiResponse::new(Json("Logged out successfully".to_string())).header(
                header::SET_COOKIE,
    ...
    pub async fn me(&self, req: &Request) -> PoemResult<Json<Value>> {
