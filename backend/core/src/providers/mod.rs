...
pub trait ProviderAdapter: Send + Sync {
    ...
    async fn chat(&self, req: &ChatCompletionRequest) -> Result<String, anyhow::Error> {
        ...
        eprintln!(
            "Chat collection complete: {} chunks, {} total bytes",
            chunk_count,
            content.len()
        );
    ...
    async fn transcribe_audio(
        ...
        Err(anyhow::anyhow!(
            "Speech-to-text not supported by this provider"
        ))
    }
    ...
    async fn speech_to_speech(
        ...
        Err(anyhow::anyhow!(
            "Speech-to-speech not supported by this provider"
        ))
    }
    ...
    async fn poll_video_job(&self, _job_id: &str) -> Result<serde_json::Value, anyhow::Error> {
        Err(anyhow::anyhow!(
            "Async video generation not supported by this provider"
        ))
    }
    ...
    async fn get_video_content(&self, _generation_id: &str) -> Result<Vec<u8>, anyhow::Error> {
        Err(anyhow::anyhow!(
            "Video content fetching not supported by this provider"
        ))
    }
}
