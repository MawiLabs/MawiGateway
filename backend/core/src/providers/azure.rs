...
impl ProviderAdapter for AzureProvider {
    async fn stream_chat(&self, req: &ChatCompletionRequest) -> Result<ChatStream> {
        ...
        let url = format!(
            ...
            self.base_url, req.model, self.api_version
        );
    ...
    async fn generate_image(
        ...
        let url = format!(
            ...
            self.base_url, req.model, api_version
        );
    ...
    ) -> Result<crate::types::VideoGenerationResponse> {
        eprintln!(
            "🎬 Azure Sora video generation - deployment: {}, prompt: {}",
            ...
            if let Some(gens) = generations {
                if let Some(first_gen) = gens.first() {
                    ...
                    let video_url = format!(
                        "{}/openai/v1/video/generations/{}/content/video?api-version=preview",
